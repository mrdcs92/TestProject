function doAJAX() {
	...
	xhttp.open("get", "/TestApp/asdfsdaf.do", true);
	xhttp.send();
}
	...
	xhttp.onreadystatechange = function () {
		if (this.readyState == 4 & this.status == 200) {
			...
			empJSON.forEach((employee) => {
				$("#test").append("<p>" + employee.eid + " " + employee.username + " " + employee.epassword + " " + employee.email + "<p>");
			});
		}
	};
	xhttp.open("get", "/TestApp/getall.do", true);
	xhttp.send();
}
...
function submitForm() {
	...
	xhttp.open("post", "/TestApp/tryLogin.do", true);
